...
{	
			...
			foreach ($connections as $section => $tags)
				foreach ($tags as $tag => $tag_data)
					foreach ($tag_data['profiles'] as $rel => $profiles)
					{
						...
						{							
							if (isset($dir_data[$profile]) && isset($this->affiliations[$dir_data[$profile]['ds_affiliation'][0]]))
							{
	...
	{				
		...
		if ($results = $es->run_one())
		{
			...
			{
				/* Carleton-specific */
				$filter_parts[$result->id()] = sprintf('(ds_guid=%07s)', $result->get_value('user_guid'));
				$guid_to_profile_id[$result->get_value('user_guid')] = $result->id();
			}
		}
		...
		$ds->search_by_filter($filter, array('ds_guid','ds_fullname','ds_affiliation','ds_classyear'));
		$records = $ds->get_records();
		foreach ($records as $record)
		{
			$records_by_id[$guid_to_profile_id[(int)$record['ds_guid'][0]]] = $record;	
		}
